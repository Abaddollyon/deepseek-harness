...
def compare_snapshot_files(
    ...
    filenames: tuple[str, ...],
    *,
    writer_oracles: bool = False,
) -> None:
    """Compare complete outputs; opted-in writer oracles keep canonical Session generations read-only."""
    ...
    if writer_oracles:
        for name in files:
            ...
            if parsed is not None:
                index = parsed[0]
                ordinal = "" if index == 0 else f".{index}"
                writer_paths[index] = directory / f"writer{ordinal}.expected.jsonl"
        expected_writers = {path.name for path in writer_paths.values()}
        existing_writers = {path.name for path in directory.glob("writer*.expected.jsonl") if path.is_file()}
        if existing_writers != expected_writers:
            raise AssertionError(
                f"{scenario} writer oracle roles differ: "
                f"missing={sorted(expected_writers - existing_writers)}, "
                f"unexpected={sorted(existing_writers - expected_writers)}"
            )
        for path in writer_paths.values():
            assert_current_session_version(
                session_header_version(path.read_text(encoding="utf-8"), path.name), path.name,
            )
    if update and not writer_oracles:
    ...
    } | {path.name for path in writer_paths.values()}
    ...
    if update and writer_oracles:
        for name, content in files.items():
            parsed = parse_snapshot_session_filename(name)
            destination = directory / name if parsed is None else writer_paths[parsed[0]]
            destination.write_text(content, encoding="utf-8", newline="\n")
        print(f"smoke-python-runtime: updated writer oracles and sidecars in {directory}")
    for name, actual in files.items():
        parsed = parse_snapshot_session_filename(name)
        expected_path = directory / name if parsed is None else (
            writer_paths[parsed[0]] if writer_oracles else selected_expected[parsed[0]]
        )
        expected_text = expected_path.read_text(encoding="utf-8")
        compared_actual = normalize_snapshot_comparison_text(name, actual)
        comparison_name = name if writer_oracles and parsed is not None else expected_path.name
        compared_expected = normalize_snapshot_comparison_text(comparison_name, expected_text)
