...
`sendMessage()` validates peer membership, appends `team/message/queued`, and flushes before attempting delivery. The target message begins with `Team message <id> from <name>:` and keeps the same id and sender in `TeamMessageSource`. A target receipt is acknowledged with `team/message/delivered` only after the target Session durably holds the message identity in its pending inbox or recorded history. Immediate admissions are serialized per target in durable queue order; recovery dispatches queued-minus-delivered records in the same order. A recovery pass with candidate records flushes the Lead Session inside the Lead transaction before it captures and claims them, so a record whose queued flush is still pending or has rejected is never delivered by recovery; a start with no candidate records pays no flush. That flush is a durability gate only when a persistence backend participates in `session/flush`; without one the operation is the same in-memory contract as every other Team flush. A rejected send is an unknown outcome, not a guarantee of non-delivery: persistence retains and retries the rejected write, and once a later flush succeeds a later pass delivers the record. A sender registers its own immediate dispatch inside its transaction, before any recovery pass can observe the record, so only recovery can ever find a record already in flight and `accepted` versus `queued` reports the sender's own attempt. Delivery folds both live and persisted target inbox/history state before retrying, so a crash between inbox acceptance and model claim does not duplicate the message. The guarantee is process-local retry plus target-Session de-duplication, not cross-process exactly-once delivery.
