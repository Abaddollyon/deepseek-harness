...
The tunnel's synthetic HTTP responses run through the real Web carrier response policy. They support mutable headers, `readable-stream` event semantics, and write/end callbacks. Successful callbacks precede `finish` and `close`; cancellation settles pending callbacks and prevents further response frames. Owner-local regressions apply the production policy directly to the synthetic exchange.
...
- **Synthetic HTTP supports the current Web routes and response policy, not general Node networking**: writes go directly to the tunnel sink without socket backpressure; real socket APIs remain unavailable.
- **Worker-host and page-half acceptance needs a browser-grade harness** — broader transport coverage remains incomplete; unit specs cover synthetic HTTP with the production response policy, storage, ALS, the transform, and the stub contracts.
