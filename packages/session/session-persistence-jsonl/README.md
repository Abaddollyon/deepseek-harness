...
```
<root>/
  session-index.json              # transactional per-root SessionHeader index
...
- `session-index.json` maps each id to its absolute transcript path, project directory, immutable `SessionHeader`, and stat freshness fields. `list()` and `listSnapshots()` validate freshness with metadata reads and use indexed headers for unchanged artifacts, so listing does not decompress every transcript.
...
- **Transactional summary index.** Materialization, append, and crash repair refresh `session-index.json` through an `fsync`ed temporary file and atomic rename beside the project directories. A missing index is built lazily; changed or removed transcripts repair only their entries, while newly discovered transcript directories are added without rereading indexed headers. Invalid JSON, invalid fields, a compression mismatch, or any format other than `JSONL_INDEX_FORMAT_VERSION` logs a warning and rebuilds from artifacts without migration. Id lookup uses the indexed path and scans all project directories only after an index miss.
