...
| `listConcurrency` | positive integer (default `32`) | Maximum concurrent per-session header reads during `list()`/`listSnapshots()`. Raise for high-latency filesystems (network mounts), lower to bound I/O pressure on congested disks. |
