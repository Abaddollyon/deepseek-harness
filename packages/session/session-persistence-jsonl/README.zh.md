...
- `session-index.json` 将每个 id 映射到其 transcript 绝对路径、项目目录、不可变 `SessionHeader` 和 stat 新鲜度字段。`list()` 与 `listSnapshots()` 通过元数据读取验证新鲜度，并为未变更产物使用索引 header，因此列表操作不必解压每个 transcript。
...
默认产物是独立 [Zstandard frame](../../../.agents/notes/implemented/architecture/2026-07-19-zstandard-jsonl-session-logs.md) 的标准拼接：一个仅包含 header 行的带 checksum frame，后跟每个持久 append 批次一个带 checksum frame。后端使用 Node 内置 Zstandard API 和默认压缩级别，不提供级别开关。索引重建只读取并验证每个 header frame；普通索引列表不会解压 transcript frame。`compression: 'none'` 在原始表示中保留相同逻辑行。
...
- **事务式摘要索引。** 实体化、append 和崩溃修复通过已 `fsync` 的临时文件与原子 rename，在项目目录旁刷新 `session-index.json`。缺失索引会延迟构建；变更或移除的 transcript 只修复对应条目，新发现的 transcript 目录则无需重读已索引 header 即可加入。无效 JSON、无效字段、压缩不匹配或任何非 `JSONL_INDEX_FORMAT_VERSION` 的格式都会记录警告并从产物重建，不执行迁移。id 查找使用索引路径，只有索引未命中后才扫描全部项目目录。
