...
三个必写点总是写入：会话创建保存由种子派生的切面，`turn/end` 保存列表读取所需的轮次终值，会话释放保存最终实时切面。其间，配置的条数与间隔节流随事件累积写入。写入在等待日志刷盘之前按快照顺序进入逐会话队列，因此延迟完成的创建刷盘不会覆盖后续事件或会话释放的检查点。每次写入通过领域写入链以原子方式替换该会话的完整记录；失败会记录警告并让缓存保持陈旧，后续写入会自行修复。
...
冷历史缓存修复通过 `writeBack` 携带恢复时使用的精确检查点行。按 Session 串行化并执行比较并交换，防止较旧的冷读取结果覆盖并发写入的实时检查点；失败保持软失败。包含中断轮次合成结束事件的只读结果不得持久化对应投影行。
