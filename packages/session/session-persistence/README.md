...
| `readTail(id, beforeSeq, limit, signal?): Promise<{ meta; events; hasMore }>` | Return at most `limit` valid stored events immediately before the exclusive `beforeSeq`, or the latest durable tail when the bound is omitted. Events remain seq-ascending and `hasMore` reports whether an older valid event exists. SQLite seeks over bounded physical rows; sequential providers use the complete-prefix fallback. |
...
| `loadStoredTail?(id, beforeSeq, limit, signal?)` | Optional bounded reverse-page read behind `readTail`; returns a seq-ascending logical page and exact older-event indicator without materializing the complete log. |
...
- **`list()` is unpaginated and unfiltered** — it returns every stored session's header; first-party stores use metadata indexes but still allocate the complete result.
