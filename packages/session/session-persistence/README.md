...
| `readTail(id, beforeSeq, limit, signal?): Promise<{ meta; events; hasMore }>` | Return at most `limit` valid events before the exclusive sequence bound, or before the durable end when the bound is absent, plus whether an older event remains. The default implementation uses `readFrom(id, 0)`, so sequential backends may scan the complete artifact while still bounding the returned page. |
