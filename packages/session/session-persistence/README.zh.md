...
| `readTail(id, beforeSeq, limit, signal?): Promise<{ meta; events; hasMore }>` | 返回排在独占序号界限之前的至多 `limit` 个有效事件；省略界限时从持久尾部读取，并报告是否仍有更早事件。默认实现使用 `readFrom(id, 0)`，因此顺序后端可能仍会扫描完整工件，但返回页保持有界。 |
