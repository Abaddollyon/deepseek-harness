...
The Tab domain retains navigation, an abort signal, and bound actions per (Session, tab id). Either store-backed seat's injection adopts the exact store instance under its native Session id before the seat can commit an action. The renderer's compound Host/Session storage key remains opaque; each environment-owned controller retains its own occurrences even when Hosts share native ids. Adoption is shared across both seats and reconciles records on store commits, including offscreen actions. Only record removal or plugin unload aborts the signal; closing the sidebar and switching Sessions retain records, while undo restores a new occurrence. `useTabInfo()` composes framework-bound store and navigation hooks without manual component subscriptions or render-time record creation. `tab.actions` always target their own Session; `tab.visible` distinguishes bodies from titles, and floating tabs remain visible when the sidebar closes. `adopt` is absent from the public controller.
