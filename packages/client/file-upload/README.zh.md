...
远程 Environment runtime 的优先级高于页面全局钩子与 fixture 模式：原始上传使用其注册的功能请求载体，保留 Host 标识、取消与 generation 检查。若完整响应体到达前 Host 重新连接或失去就绪状态，上传会拒绝该结果，即使响应头已在原 generation 中到达。本地 Environment 仍使用页面拥有的 Worker 或 Fetch 载体。远程组合必须将本包与其自己的 `remote` 和 `environmentRuntime` 服务一起挂载；共享本地上传服务会寻址到错误的 Host。自定义载体不报告字节进度。
