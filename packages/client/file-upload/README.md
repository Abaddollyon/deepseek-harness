...
A remote Environment runtime takes precedence over page-global hooks and fixture mode: raw uploads use its registered feature request carrier, retaining Host identity, cancellation, and generation checks. An upload rejects its result if the Host reconnects or loses readiness before the complete response body has arrived, even when the response headers arrived on the original generation. The local Environment retains the page-owned Worker or Fetch carrier. Remote composition must mount this package with its own `remote` and `environmentRuntime` services; sharing the local upload service would address the wrong Host. Custom carriers do not report byte progress.
