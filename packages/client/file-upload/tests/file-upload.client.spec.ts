...
describe('environment-owned uploads', () => {
  it.each(['blob', 'stream'] as const)('keeps %s uploads on the remote Host and releases its route', async (kind) => {
    ...
    const request = vi.fn(async () => Response.json({
      ok: true,
      value: { receiptId: 'remote-receipt', file: { attachmentId: 'remote-file', name: 'data', bytes: 1 } },
    }))
    const release = vi.fn()
    const registerFeatureRoute = vi.fn(() => release)
    const ctx = new Context()
    ctx.provide('environmentRuntime', {
      environmentId: 'remote-host', runtimeId: 'remote-runtime',
      generation: { getSnapshot: () => undefined, subscribe: () => () => {} },
      request, registerFeatureRoute,
    })
    const fiber = ctx.plugin(FileUploadRuntime)
    await fiber.await()
    const body = kind === 'blob' ? new Blob(['x']) : new ReadableStream<Uint8Array>({
      start(controller) { controller.enqueue(Uint8Array.of(1)); controller.close() },
    })
    const signal = new AbortController().signal
    try {
      await expect(ctx.fileUpload.upload('shared-id' as SessionId, body, 'data', signal))
        .resolves.toMatchObject({ ok: true, value: { receiptId: 'remote-receipt' } })
      expect(registerFeatureRoute).toHaveBeenCalledWith('/api/session/uploadFileBinary')
      expect(request).toHaveBeenCalledWith('/api/session/uploadFileBinary?sessionId=shared-id&name=data', {
        method: 'POST', body, signal, headers: { 'content-type': 'application/octet-stream' },
        ...(kind === 'stream' ? { duplex: 'half' } : {}),
      })
