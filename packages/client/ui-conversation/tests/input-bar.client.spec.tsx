...
describe('running and lock semantics', () => {
  ...
  it('blocks attachment intake during disconnect without discarding local drafts', () => {
    const connectionReady = createSnapshotStore(false)
    const addFiles = vi.fn(() => null)
    const result = bench({ draft: 'retain me', connectionReadySource: connectionReady, addFiles })
    const file = new File(['draft'], 'draft.txt', { type: 'text/plain' })
    const owner = attachmentOwner(result.slotCalls)
    expect(owner.canAcceptDrop).toBe(false)
    expect(result.view.container.querySelector<HTMLButtonElement>('button[aria-label="添加附件"]')?.disabled).toBe(true)
    act(() => { owner.onAddFiles([file]) })
    expect(addFiles).not.toHaveBeenCalled()
    expect(result.shell.snapshot.draft).toBe('retain me')

    act(() => { connectionReady.set(true) })
    expect(attachmentOwner(result.slotCalls).canAcceptDrop).toBe(true)
    act(() => { attachmentOwner(result.slotCalls).onAddFiles([file]) })
    expect(addFiles).toHaveBeenCalledWith([file])

    const connectedOwner = attachmentOwner(result.slotCalls)
    act(() => { connectionReady.set(false) })
    act(() => { connectedOwner.onAddFiles([file]) })
