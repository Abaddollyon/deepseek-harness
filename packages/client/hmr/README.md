...
- **Only the rebuilt row's rev is refreshed** — a `rebuilt` frame moves that one row to the hash it names, which is what keeps the refetch out of the browser's immutable cache; the graph-wide rev anchor still goes stale until a reconnect refreshes it.
