...
describe('WorkspaceBrowser', () => {
  ...
  it('defers the reveal until the Workspace baseline says which group renders the session', async () => {
    ...
    expect(b.store.getSnapshot().groupExpansion).toEqual({})
    expect(groupHeader('未分组').getAttribute('aria-expanded')).toBe('false')
  ...
  it('shows only the current blank session as the localized New Session, excluded from search', async () => {
    ...
    // The blank row is the current session, so the navigation reveal opens the
    // group that renders it.
    await waitFor(() => { expect(screen.getByText('新会话')).toBeTruthy() })
