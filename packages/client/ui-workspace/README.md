...
This package lets users browse grouped or flat Session lists, choose a Workspace or **Don't use a workspace** for a new Session, and manage Workspaces and Sessions through add, rename, reorder, search, fork, archive, and Workspace deletion. Workspace-free conversations appear in the **Chats** group. Pending interactions appear as warning dots, active scheduled tasks as alarm markers, and subagent-origin Sessions remain hidden. Canonically distinct folder paths remain separate Workspaces. Adding a Workspace requires a composed directory picker; without one, the add action is unavailable.
...
Use the sidebar to browse Workspaces and their Sessions, reorder them, and start new ones; use the picker in the Session Intent hero to choose a Workspace or start a separate workspace-free Session. The sidebar's add-only control remains directory-only. Workspace-free conversations appear under **Chats**, while the persisted ungrouped account keeps its stable internal key. An open Workspace shows five non-blank Sessions by default and keeps the selected blank **New Session** as one provisional extra row until its first prompt. The provisional row retains its ordered position without reducing that quota or inflating the hidden count. **Show more** reveals the hidden remainder; closing and reopening the Workspace restores this folded projection.
...
Once the Workspace list baseline is ready, browser-persisted expansion and Session-order records retain only current Workspace ids plus Chats and the flat-list account. Real Workspaces initialize from `WorkspaceView.sessionIds`, while Chats and the cross-Workspace flat list initialize from recency. The shared sidebar projection hides rows whose durable Session summary has `origin: 'subagent'`, and each visible ordinary row inherits the blue activity indicator while any descendant reached through uninterrupted subagent-origin lineage is running. The same pure derivation reads the Schedule key from list projection values for grouped, flat, and search nodes; the package uses only the type-only `@deepseek-ai/dsh-schedule/client` dependency and does not import the Schedule runtime or `ui-schedule`.
