...
describe('ModelSelect reasoning effort', () => {
  ...
  it('refreshes explicitly, keeps rows during the request, and preserves Escape navigation', async () => {
    ...
    await waitFor(() => { expect(document.activeElement).toBe(modelEntry) })
    fireEvent.click(modelEntry)
    await waitFor(() => { expect(document.activeElement).toBe(screen.getByRole('searchbox', { name: zh['search.aria'] })) })
    ...
    fireEvent.click(screen.getByRole('menuitem', { name: /推理等级/ }))
    await waitFor(() => { expect(document.activeElement).toBe(screen.getAllByRole('menuitemradio')[0]) })
