...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希写入启动图，并通过 `/plugins` 提供该文件及其 sourcemap。该 route 只声明媒体类型：bundle URL 携带启动图生成的内容哈希（`?rev=<hash>`），因此由 [webserver 的响应策略](../../host/webserver/README.md)将其钉住，并让没有哈希的 sourcemap 保持需要重新验证。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
