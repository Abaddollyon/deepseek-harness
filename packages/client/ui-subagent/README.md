...
Web subagent feature owner: contributes the unified session-scoped Agent flow to `conversation.view`, the lazily expandable catalog tree to `conversation.session.header.actions`, reason-specific read-only replacements to the conversation composer chain, and the existing `@` reference source to `ctx.inputTriggers`.
...
The unified session-scoped Agent flow is the persisted `swarm` entry in `conversation.view` and is present even when the current session has no descendants. It merges `byId` subagent summaries with lazy `subagentsByParent` catalogs, keeps ordinary forks out of lineage, preserves catalog-only placeholders and diagnostics, derives healthy addresses from the catalog parent key plus child id and mode, and uses retained addresses only when the runtime has one. Summary projections supply provider/model, the four disjoint token buckets, active timing, and optional turns/steps facts; missing values are localized as unavailable. Root and expanded parents own catalog observation. Structural rows are memoized separately from a one-second clock used only by visible active timings, and settled durations freeze at `active.through`. Rows expose tree semantics and stable `data-flow-id`/`data-flow-depth` metadata; diagnostics and loading placeholders cannot be opened. At rollout, this core `swarm` registration replaces the external `@arro/dsh-swarm-ui` contribution; the external source and workflow-card patch remain outside this package.
