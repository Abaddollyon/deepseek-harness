...
Web subagent 功能 owner：向 `conversation.view` 贡献统一的会话级 Agent 流程，向 `conversation.session.header.actions` 贡献可懒加载展开的目录树，向会话编辑器链贡献按原因区分的只读替代呈现，并保留注册到 `ctx.inputTriggers` 的既有 `@` 引用 source。
...
统一的会话级 Agent 流程是 `conversation.view` 中持久化的 `swarm` 条目，即使当前会话没有后代也会显示。它合并 `byId` 中的 subagent 会话摘要与按需加载的 `subagentsByParent` 目录，将普通 fork 排除在谱系之外，同时保留只有目录记录的占位行和诊断行；健康条目的地址由目录 parent key、child id 与 mode 推导，只有运行时保留的地址才使用运行时地址。摘要投影提供提供方／模型、四个互不重叠的 token 桶、活动耗时和可选的轮次／步骤事实；缺失值会本地化显示为不可用。根目录与展开的 parent 各自负责目录观察。结构行与仅供可见活动耗时使用的单秒时钟分开记忆化，已结束耗时冻结在 `active.through`。行提供 tree 语义和稳定的 `data-flow-id`／`data-flow-depth` 元数据；诊断行和加载占位行不能打开。发布时，核心 `swarm` 注册会替换外部 `@arro/dsh-swarm-ui` 贡献；外部 source 与 workflow-card patch 不属于本包。
