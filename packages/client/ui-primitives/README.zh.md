...
两条渲染路径的开销随输入无界增长，因此各自设有上限，且两者都是部署值而非常量。`configureHighlighting({ maxSourceChars, cacheEntries })` 与 `configureMarkdownRendering({ settledCacheEntries })` 负责安装上限，并返回恢复此前取值的 disposer；由持有方插件（[`ui-conversation`](../ui-conversation/README.md)）在插件 effect 中调用，本包不读取任何配置。内置值为 10,000 个字符、每种输出形式 128 条高亮结果、256 份定稿 Markdown 渲染。

分词是一次同步的主线程任务，开销与源文长度成线性关系——实测一个 20,578 字符的 TSX 文件耗时 663 ms——因此超过 `maxSourceChars` 时 `CodeBlock` 与 `ReadBlock` 渲染等宽的纯文本回退，而不是冻结标签页；调高上限是用交互延迟换取大体量表面的高亮。高亮结果与定稿 Markdown 元素树保存在按最近最少使用淘汰的缓存中，键为源文（Markdown 还包含 `codeLabels` 与 `fileMentions` 这两个对象的标识，因为产出的元素捕获了它们的回调），因此重新挂载是一次查表而非重新解析。惰性语法的加载通知按语法分发：`python` 模块落地只会让用 python 书写的表面重新渲染，而不会波及所有已挂载的代码块。
...
- **语法高亮仍在主线程运行**：上限与结果缓存限制并摊薄了开销，但一次未超限的分词仍是同步任务（桌面端实测每 1,000 个 TSX 字符约 10 ms）。把分词移入 worker、先渲染纯文本回退、在解析完成后换入 token runs，是尚未完成的结构性修复；按语法的加载订阅已经就是它要复用的换入通道。
- **定稿 Markdown 缓存按条目数而非字节数设限**：保留内存等于上限乘以部署中出现的最大单条消息。为超长会话调整该值是部署决策，不会自动发生。
