...
describe('dsh-tool-subagent', () => {
  ...
  it('passes persona/toolFilter/maxDepth config through to the start request', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'capture2',
      capabilities: { outputSchema: false, depthLimit: true, toolFilter: true, persona: true, agentOptions: false },
      inheritsParentContext: false,
      start: async (request) => {
        seen = request
        return {
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('forwards job_kill reasons through the run signal (and defaults one when absent)', async () => {
    ...
    const withReason = await ctx.tools.execute({ signal: testToolSignal, callId: ToolCallId('k1'), name: 'job_kill', arguments: { job_id: jobOne, reason: 'superseded' }, agent: parent })
    const withoutReason = await ctx.tools.execute({ signal: testToolSignal, callId: ToolCallId('k2'), name: 'job_kill', arguments: { job_id: jobTwo }, agent: parent })
    ...
    const killed = await ctx.tools.execute({ signal: testToolSignal, callId: ToolCallId('w1'), name: 'job_output', arguments: { job_id: jobOne, wait: true }, agent: parent })
...
describe('dsh-tool-subagent continuable background mode', () => {
  ...
  it('isolates a cancelled continuable preparation from a concurrent sibling', async () => {
    ...
    ctx.subagents.registerProvider({
      name: 'gated',
      capabilities: { outputSchema: true, depthLimit: true, toolFilter: true, persona: true, agentOptions: false },
      inheritsParentContext: false,
