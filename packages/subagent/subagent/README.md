...
Continuable teardown completes child-first handle disposal before reporting failures. Its `ACTIVATION_TEARDOWN_FAILED` errors retain descendant failures through nested `AggregateError` causes, preserving typed quota or rate-limit classification and retry delays for callers and parent settlement notices.
