...
`session.history` 会读取已附加 Session 的内存状态，或通过持久化读取冷日志，而不会恢复或发布 agent，然后按追加来源的消息边界分页。冷读取从 256 个原始事件开始，并将窗口逐次翻倍（单次读取最多 16,384 个事件），直到窗口包含请求的消息边界；开放尾部或陈旧的投影水位线会回退为一次按 id 的完整检查。`maxMessages` 统计以追加方式进入 surface 的 `user/message` 和 `assistant/message` 事件，因此仅供模型使用的替换副本不占用配额。每一页仍是一段连续的原始事件区间，从而让压缩（compaction）的仅日志 `compaction/summary` 记录与引用它的替换留在同一页。
