...
import FilesystemDirectoryBrowser, { boundedInsert, fullyQualified, raceAbort } from '../src/index.ts'
import type { ListingCandidate } from '../src/index.ts'

let root: string
let capability: DirectoryBrowser
...
beforeAll(async () => {
  ...
  const fiber = ctx.plugin(FilesystemDirectoryBrowser)
  await fiber.await()
  capability = ctx.get('directoryBrowser')!
...
describe('FilesystemDirectoryBrowser', () => {
  ...
  it('cuts a level at maxEntries keeping the name-sorted head, and flags the cut', async () => {
    const ctx = new Context()
    const fiber = ctx.plugin(FilesystemDirectoryBrowser, { maxEntries: 1 })
    await fiber.await()
    const bounded = ctx.get('directoryBrowser')!
    try {
