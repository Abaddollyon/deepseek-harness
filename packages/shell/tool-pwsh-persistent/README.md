...
- **Input echo is unavoidable**: PowerShell's line editor renders submitted input back into the terminal stream, and removing PSReadLine does not suppress that rendering. Marker-anchored extraction excludes the echo in complete results, and the wrapper-source match spans physical line breaks in fallback paths. If terminal scrollback clips part of the wrapper before capture, the unmatched fragment can remain in an incomplete result bounded by `maxOutputChars`.
