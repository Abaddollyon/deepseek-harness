...
const client: DshClientManifest = { platform: 'web', defaultRoot: false }
...
`client.defaultRoot: false` excludes a package from ordinary root selection; an explicit Client surface or another package's dependency may still include it. Omission preserves default-root inclusion. [Client modules](../../client/modules/README.md) owns the runtime graph rules.
