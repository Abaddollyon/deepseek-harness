    ...
    if (!Array.isArray(record.phases)) {
      ...
      record.phases.forEach((phase, index) => {
        ...
        for (const key of Object.keys(entry)) {
          if (!['title', 'detail', 'provider', 'model', 'reasoningEffort'].includes(key)) violations.push(`meta.phases[${index}].${key} is not a recognized field`)
        }
        ...
        if (entry.reasoningEffort !== undefined && typeof entry.reasoningEffort !== 'string') violations.push(`meta.phases[${index}].reasoningEffort must be a string`)
