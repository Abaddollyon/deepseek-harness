...
- `read(id, caller?)` consumes the single cursor for stream jobs and reads terminal output idempotently for final-output jobs while the record stays retained.
...
Owned access compares the job's `SessionId` with the caller's. Ids such as `bash-1` are predictable, so this fence is the boundary. Unowned jobs are open to callers and last until bounded-retention eviction or service disposal.
...
Implementations also owe the lifecycle semantics of the contract: registrations outlive producer and controller fibers, owner and service disposal cancel live work and await compliant producers (an implementation may bound that join with a validated grace, force-settling producers that never settle), and settlement is first-wins — one terminal record, one round of contained listener notification, released waiters. An implementation may bound how many terminal records stay retained; an evicted record reads as an unknown id, and its removal is announced through `onJobsChanged`.
