...
1. Enumerate the store's non-terminal records whose `incarnation` differs from `PROCESS_INCARNATION`. Same-incarnation records are live in-process work — an HMR reload must never mistake them for orphans — and terminal records need no driving.
