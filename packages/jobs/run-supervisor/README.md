...
1. Enumerate the store's running records whose `incarnation` differs from `PROCESS_INCARNATION`. Same-incarnation records are live in-process work — an HMR reload must never mistake them for orphans — and stopping records are durable cancellation state that never re-enters resume.
...
Events reach the owner session through whichever lane can: the live session append when the agent is registered, else a durable offline append through `sessionPersistence` at the log's next seq (retried through the live lane if the session comes live mid-append). A log that already carries a run/* event for the job is left alone, so re-boots never duplicate the account.
