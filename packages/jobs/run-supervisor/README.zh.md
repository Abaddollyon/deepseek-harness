...
4. 收养本身属于生产方：返回 hooks 的 `registerResumer` 处理器以原 id 重新收养记录，supervisor 观测到这一点（store 记录翻转为当前 incarnation）并记为 `run/resumed`。拒绝或抛错的 resumer 记为 `reason: 'resume-failed'` 的 `run/abandoned`。
