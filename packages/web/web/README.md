...
| `searchMany(request, signal?)` | Resolve the search provider once and run an ordered batch. Calls optional provider-native `searchMany` once or uses bounded legacy `search` calls. Returns one safe result-or-error outcome per query and normalizes/deduplicates HTTP(S) sources. |
...
The failure branches throw `WebError`, whose structured code (plus message detail — the missing id, the ambiguous candidate set) is the direct callers route on. A provider's own `available()` is a cheap capability/authentication check that feeds this execution-time selection and **must not spend a search request**; `dsh-tool-web` never calls it — the tool executes through `ctx.web.search()`/`fetch()` and routes on the thrown codes, so provider selection has one owner.

## Search coordination config

| Key | Default | Meaning |
|---|---:|---|
| `searchMode` | `cached` | Freshness mode supplied when a caller omits one. |
| `searchCacheTtlMs` | `300000` | Success-cache lifetime for `cached` and `indexed` queries. |
| `liveSearchCacheTtlMs` | `0` | Success-cache lifetime for `live` queries; zero retains only in-flight singleflight work. |
| `searchCacheMaxEntries` | `512` | Host-scoped LRU bound for successful per-query results. |
| `nativeBatchConcurrency` | `2` | Active native provider batches; valid values are one or two. |
| `legacySearchConcurrency` | `4` | Calls inside one legacy provider batch. |

The Host-scoped coordinator keys each query by provider id, freshness mode, normalized domain controls, location, and result cap. It returns partial cache hits immediately, submits only misses in a native batch, and singleflights overlapping misses across concurrent agents. Failures are never cached. At most two native provider batches run at once; cancelled queued batches never reach a provider. Each caller owns one waiter reference: cancelling one waiter preserves shared work, while the last cancellation aborts the provider-owned signal and waits for the provider to settle before returning `WEB_ABORTED`. Service disposal aborts queued and active work and awaits quiescence. Debug telemetry contains only aggregate provider id/count/timing facts; it never includes queries, domains, locations, result text, or URLs.

## Vocabulary

`WebSearchRequest` (`query`, `maxResults?`) → `WebSearchResult` (`content?`, `sources[]`, `truncated`); `WebSearchBatchRequest` adds ordered `queries`, freshness mode, domain filters, and location, and returns ordered `WebSearchBatchOutcome` values with either a result or a safe machine-routable error; each `WebSearchSource` has a required `url` and optional `title`/`snippet`/`publishedAt` (Perplexity citations may be URL-only). `WebFetchRequest` (`url`) → `WebFetchResult` (final `url`, `statusCode`, `body`, `truncated`); cancellation is an optional `AbortSignal` argument to `search()`/`searchMany()`/`fetch()`; coordinated search links it to a refcounted provider-owned signal. `WebFetchBody` is a CLOSED discriminated union (`html` | `text`) owned here — consumers `switch` to exhaustiveness so a new kind breaks their compilation until handled. See `src/types.ts` for the full contracts and the `WebError` code taxonomy.
...
- **Single-query `WebSearchRequest` carries only `query` + `maxResults`** — provider-neutral freshness, domain, and location controls belong to batch-native providers through `WebSearchBatchRequest`; legacy providers receive only the controls their existing contract can honor.
