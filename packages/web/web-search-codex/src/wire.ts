...
import type { WebSearchBatchRequest } from '@deepseek-ai/dsh-web'
...
function threadConfig(request: WebSearchBatchRequest): JsonObject {
  return {
    web_search: request.mode === 'live' ? 'live' : 'cached',
...
function batchPrompt(request: WebSearchBatchRequest): string {
  return [
    'Use the built-in web search tool exactly once for each query below and use no other tools.',
    'Search every query exactly as written. Do not merge, rewrite, omit, or reorder queries.',
    request.allowedDomains === undefined
      ? undefined
      : `Use sources only from these allowed domains: ${JSON.stringify(request.allowedDomains)}.`,
    request.blockedDomains === undefined
      ? undefined
      : `Do not use sources from these blocked domains: ${JSON.stringify(request.blockedDomains)}.`,
...
export class CodexSearchWire {
   ...
   * @param signal - operation cancellation signal.
   */
  async startThread(cwd: string, request: WebSearchBatchRequest, signal: AbortSignal): Promise<void> {
    const response = object(await this.guarded(this.transport.request('thread/start', {
      cwd,
      ephemeral: true,
      approvalPolicy: 'never',
      sandbox: 'read-only',
      config: threadConfig(request),
   ...
   */
  async runTurn(request: WebSearchBatchRequest, signal: AbortSignal): Promise<CodexSearchTurnResult> {
    ...
    const response = object(await this.guarded(this.transport.request('turn/start', {
      threadId,
      input: [{ type: 'text', text: batchPrompt(request), text_elements: [] }],
