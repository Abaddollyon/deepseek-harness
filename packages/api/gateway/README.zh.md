...
隔离展示 Context 中的 Remote 订阅注册到 Remote 所属事件树，并由展示 fiber 的生命周期管理；Agent 作用域事件可以送达，展示 fiber 卸载后订阅移除。
...
`ctx.remote` 不暴露 Connection 生命周期控制。只有职责包含恢复的消费方才直接读取 `ctx.connection.state` 并调用 `ctx.connection.reconnect()`；普通 Remote 消费方仍只使用生成的 namespace 与 `$stream()`。Controller feed 所有者复用 Gateway 的有序清理生命周期，确保前一条流的清理结束后才开始替换流。
