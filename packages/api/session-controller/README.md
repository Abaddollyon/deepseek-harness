...
`ctx.sessions.feed` reports loading, bounded retrying, ready, terminal error, or stale retained data separately from transport connectivity. Readiness requires a control baseline and authoritative list; only `gateway/service-unavailable` retries automatically. `retryFeed()` preserves selection, Session objects, and drafts. Host/environment identity scopes persisted selection. Reconnect rebuilds opened journals with serialized, disposal-fenced resyncs. Queued complete projection values coalesce per Session/key; queue and jobs frames retain push order.
