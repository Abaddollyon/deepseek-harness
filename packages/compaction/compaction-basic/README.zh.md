...
| `maxOverflowRetries` | `1` | 每次预检准入与提供方确认溢出序列中的最大替换重试次数；`0` 禁用两条自动路径。 |
...
最旧的平衡范围会被替换为一条摘要消息，近期尾部保持逐字不变；对话从摘要继续。操作会报告压缩了多少历史项以及估算释放的 token 数。如果没有任何内容可以安全压缩——例如整个对话就是一个不可分单元——则不会有任何改变，也不会向会话日志写入任何内容。如果没有模型可以撰写摘要（既未配置目标，也还没有已路由请求），压缩会失败并给出清晰错误，提示你配置摘要提供方与模型，或先路由一次请求。创建时带预算的 Agent 也会以 `BUDGET_ACCOUNTING_UNAVAILABLE` 拒绝本后端的模型摘要，因为这项辅助请求不在 Agent 的会话请求计量之内。该决定使用 Agent 在构造时捕获的预算存在性信号，所以之后修改其 options 也无法启用这项请求。无模型修剪仍可在不生成摘要的情况下消除压力；自动溢出恢复会保留原始提供方错误，并记录摘要被拒绝的诊断。
...
在本包之前挂载 `dsh-compaction-tool-result-pruner`，即可在压缩过程中修剪超大工具结果。修剪不发起模型调用，并可能完全省去摘要：请求预检仅在修剪后的压力低于配置阈值，且请求加上预留输出不超过模型容量时跳过摘要。没有移除内容的 pruner 不会阻止阈值触发的摘要。修剪只在压缩触发条件满足后运行——低于压力的对话绝不会被触碰。
...
每个选定范围都从第一个不是 `system/message` 的 surface 节点开始，因此位于 surface 节点 0 的系统提示词永不会被遮蔽；由历史内提示词更新追加的后续 `system/message` 是普通历史，范围可以遮蔽它，agent loop 的投影随后会在二者文本不同时用当前提示词替换节点 0（[决策规则](../../core/agent-loop/README.zh.md#understand-the-implementation)）。
