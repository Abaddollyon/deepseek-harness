...
`AgentOptions` 提供初始 provider/model 路由、可选的显式 `reasoningEffort`、可选的正数 `maxTokens` 输出上限，以及可选的完整 `budget`。该预算会限制一个实时循环实例的模型步骤数、按响应计量的输入 token、请求的输出 token 与请求错误重试。`Agent.hasExecutionBudget` 会捕获该实例创建时是否带预算，因此即使调用方之后修改保留原 identity 的 `AgentOptions` 对象，辅助模型调用的所有者仍能拒绝未计量工作。配置的推理强度会为新建循环实例的首个请求提案设定种子值，并覆盖恢复日志中的值；省略时，恢复仅会取回同一路由的显式值。后续提案会保留已记录的显式推理强度，同时重新解析标记为适配器默认值的值。循环会校验确切模型的推理支持、把有效值记录在请求头中，并将它们应用到每个对话请求。可选的 `setup(agentCtx, agent)`（显式的未发布 Agent 提供其 Session；Context 没有反向 Agent 属性） 回调会在 agent 发布之前组合其作用域世界——作用域工具、提示词段与监听器在任何创建公告之前就已存在。Setup 只做组合：创建完成后才能驱动 agent。
...
- **`agent/session-start` 不能为启动设置门禁**：它仍是同步且不可 veto 的通知；必须在发布前完成的异步组合属于工厂的 `setup(agentCtx, agent)`（显式的未发布 Agent 提供其 Session；Context 没有反向 Agent 属性） 事务。
