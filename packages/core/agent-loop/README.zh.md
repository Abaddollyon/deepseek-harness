---
description: "具体的 agent 循环驱动器与生命周期所有者。"
...
具体的 agent 插件和循环驱动器拥有 session、turn 与 step 生命周期。

## 目录

- [服务：AgentLoop](#service-agentloop-ctx-key-agentloop)
...
每个 agent 与其会话共享一个由调用方选择的 `SessionId`，并假设它在全局唯一；意外的 UUID 冲突不属于受支持模型。两个使用同一 id 的并发操作都可以进行准备，但最终的 `enter()` 调用会裁决发布，所有失败方都会回滚各自的私有资源。每次 detach 都绑定到确切进入的对象，因此陈旧 disposer 无法移除之后出现的同 id 替代项。在同步创建通知期间请求的 detach 会等待该次分发退栈，从而保留 created/disposed 配对。Teardown 按以下顺序执行：停止并排空 → 等待会话 flush → 撤销作用域 → detach agent → detach 会话，并保留待处理 inbox，以便后续生命周期恢复它。受等待的 flush 使已完成的拆卸成为驱动器收尾事件的持久性屏障；flush 失败只会发出警告而不会中断拆卸。私有作用域清理完成后，该 id 即可复用。不具否决能力的普通 `agent/*` 通知通过 `agentEvents(ctx, agent)` 发出；逐步骤组装通过 `assembleContextFor(agent)` 完成。
...
驱动器在其整个生命周期内拥有一个 agent，并在 `ctx.agents.withInitiator(agent, ...)` 内运行。包私有的编排入口点会恢复确切的 Agent，一次性派生 `agent.session`，并让操作局部的辅助函数捕获它，而不是通过浅层接口继续传递具体驱动器或每次操作的 `Session`。如果显式 `Session` 正是辅助函数的实际接口，该辅助函数会保留它；创建、持久化加载、未发布 setup、服务、worker、进程、持久化和 wire 协议则继续保留各自的显式身份。[agent 服务](../agent/README.zh.md#initiator-scope)规定传播、teardown 和分离工作规则。
...
- 压缩（compaction）：在 `agent/request-preflight` 上对精确路由执行压力准入；在 `agent/request-error` 上进行规范的溢出修复
...
生命周期变更需要覆盖取消、teardown 和持久化的专项测试。
