...
A `request/header` with `reason: 'series'` already declares a new series and omits `startsSeries`. A changed header uses `reason: 'change'` and includes `startsSeries: true` only when that change also starts a series; readers must recognize both representations.
...
After logging the exact request header and context, `agent/request-preflight` receives the frozen header and resolved capacity before history derivation. A retry must identify a newer committed replacement generation; log-only activity cannot justify it. Admission stops redispatching after eight productive retries so provider error recovery remains reachable. A replacement consolidates the current system prompt and logs a new request-series header before deriving the admitted request.
