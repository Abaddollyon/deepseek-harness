...
- id: code-runtime
  ...
  config:
    ...
    maxWallMs: 3600000            # idle ceiling; suspended while a host dispatch is outstanding
...
- **Two independent budgets, because the peer is hostile** — `computeMs` meters the worker's MEASURED busy time (`worker.performance.eventLoopUtilization()` polling): a hot loop cannot hide behind a pending decoy dispatch, and a program awaiting a slow tool accrues nothing. `maxWallMs` backstops what busy time cannot see (awaiting a promise nobody resolves) and charges only stretches with NO host binding dispatch outstanding, because a program waiting on a dispatch is waiting on sanctioned host work, not idling — the same fairness the compute budget already has. Each dispatch suspends the ceiling and each reply re-arms it with the REMAINING budget, so alternating short dispatches with idle gaps still reaches the ceiling. Both funnel into `worker.terminate()`, which ends hot synchronous loops too; heap overflow surfaces as the worker's OOM exit (`kind: 'worker-exit'`). `maxWallMs` is range-checked at load against `MAX_TIMER_DELAY_MS`: `setTimeout` clamps a longer delay to 1 ms, so a positivity check alone would accept a ceiling that expires on the first tick. `computeMs` needs no such bound, being compared against measured utilization rather than fed to a timer.
...
- **A host binding that never settles holds its run open** — the idle ceiling is suspended for as long as the dispatch is outstanding, so such a run ends only through the request's abort signal or runtime disposal; bounding an individual dispatch belongs to the consumer that owns the binding.
