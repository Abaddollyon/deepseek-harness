...
`observeSession` 的 `historyTail: true` 选项先通过只读句柄验证并迁移完整代次，再从身份匹配的投影检查点折叠后缀。后缀扩大到覆盖请求消息页及其来源事件，返回前重新检查持久化 revision 与实时挂载，并等待比较并交换缓存修复。中断轮次的合成结束事件保持只读，不进入持久投影检查点。首次打开仍需解码完整代次；检查点仅减少投影折叠工作。
