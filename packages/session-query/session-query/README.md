...
`observeSession({ historyTail: true })` uses a read handle to validate and migrate the complete generation before projecting an identity-checked checkpoint suffix. It widens that suffix until the requested message page and its source events fit, rechecks the stored revision and live attachment, and awaits compare-and-set cache repair before returning. Synthetic interrupted-turn closers remain read-only and never enter durable projection checkpoints. First-open storage decoding is still whole-generation work; checkpoint reuse bounds projection folding, not physical decoding.
