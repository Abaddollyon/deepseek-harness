...
describe('TokenMeter pricing', () => {
  it('prices only tool schemas through the retained request-header estimator', () => {
    const service = meter()
    const session = Session.create(SessionId('header-price'))
    const envelope = header('model', { tools: [READ_TOOL] })
    appendHeader(session, envelope)
    const toolTokens = service.estimateHeader(envelope)
    expect(service.estimateHeader(undefined)).toBe(0)
    expect(service.estimateHeader(header('model'))).toBe(0)
    expect(toolTokens).toBeGreaterThan(0)
    expect(service.measure(session).totalTokens).toBe(toolTokens)
    appendSystem(session, 'system prompt')
    const measurement = service.measure(session)
    expect(service.estimateHeader(envelope)).toBe(toolTokens)
    expect(measurement.totalTokens).toBe(toolTokens + measurement.surfaceTokens)
  })
