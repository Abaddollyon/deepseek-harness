...
使用 `ctx.tokenMeter` 估算会话当前的请求与上下文压力，或为单条消息计价。测量会回放持久会话日志，结果确定且不进行模型调用，因此压缩、占用显示与遥测可以共享同一结果。可选的会话投影提供用量、压力、上下文构成与最近解析出的模型路由。本包不添加模型可见内容，也不为 loop 做决定。
...
`ctx.tokenMeter` 暴露三个操作。`measure(session, requestHeader?)` 在同一个已消费日志 revision 上返回独立、深度不可变的快照：`totalTokens` 是请求与响应压力，`surfaceTokens` 是仅表面的路由定价总量，等于 `nodes[].tokens` 之和。可选 `requestHeader` 覆盖会选择计价路由与压力字段；节点集合仍描述当前会话。`estimateMessage(message)` 用固定启发式规则为一条消息计价。`estimateHeader(header)` 仅对请求 envelope 中的工具 schema 计价；V3 系统消息作为 surface 节点单独计价。每次调用都会克隆带位置的表面节点，因此测量是 O(surface)。
...
当组合提供 `ctx.sessionProjections` 时，token-meter 注册四个投影单元。`tokenUsage` 携带完整持久日志中的 `uncachedInputTokens`、`outputTokens`、`cacheReadTokens` 与 `cacheWriteTokens`。每条持久 `assistant/message` 或 `assistant/attempt` 都贡献该次尝试的最终用量样本；`llm/retry-started` 会结束该替换范围，因此同一步骤中的重试会贡献另一次计费用量。`contextPressure` 携带可选 `pressureTokens`（提供方报告的最新提示词规模）、可选 `projectedTokens`（下一个请求的提示词将花费多少）与来自最新一条 `request/context` 记录的可选 `contextWindow`。`contextBreakdown` 携带启发式 `systemTokens`、`toolsTokens` 与 `messageTokens`——上下文的构成，而非提供方计费规模。`modelRoute` 报告最近解析出的提供方、模型与可选上下文窗口，首次请求前为 `null`。卸载插件会移除全部四个键。
