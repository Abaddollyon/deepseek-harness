...
describe('LlmRuntime', () => {
  ...
  it('binds adapter-owned capabilities and dispatch to one prepared generation', async () => {
    ...
    const adapter = new class extends ScriptedAdapter {
      override prepareCall(provider: string, model: string) {
        ...
        return Promise.resolve({
          ...
          countInputTokens: () => captured === 'first' ? 12 : 24,
    ...
    expect(prepared.countInputTokens({ ...prepared.config, messages: [] })).toBe(12)
    expect(() => prepared.countInputTokens({ ...prepared.config, model: 'other', messages: [] }))
      .toThrow(expect.objectContaining({ code: 'INVALID_PREPARED_CALL' }))
  ...
  it.each([undefined, 0, 17, -1, 1.5, Number.NaN, Number.POSITIVE_INFINITY])(
    'validates exact prepared input-token count %s without dispatching',
    async (count) => {
      const ctx = new Context()
      await ctx.plugin(LlmRuntime)
      const adapter = new class extends ScriptedAdapter {
        override countInputTokens(): number | undefined { return count }
      }(SCRIPT)
      ctx.llm.registerAdapter(['route'], adapter)
      const prepared = await ctx.llm.prepareCall({ provider: 'route', model: 'model' })
      const countInput = () => prepared.countInputTokens({ ...prepared.config, messages: [] })
      if (count === undefined || Number.isSafeInteger(count) && count >= 0) {
        expect(countInput()).toBe(count)
      } else {
        expect(countInput).toThrow(expect.objectContaining({ code: 'INVALID_TOKEN_COUNT' }))
      }
