...
当提供方无法在分发前证明请求大小时，适配器不会返回精确计数；要求硬性预检计量的调用方必须拒绝该请求，或使用响应中的权威 usage 做后续阈值决策。
