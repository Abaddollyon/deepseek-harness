...
An adapter returns no exact count when its provider cannot prove request size before dispatch; callers that require hard preflight accounting must refuse that request or use authoritative response usage for a later threshold decision.
