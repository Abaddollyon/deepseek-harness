...
- pi-ai 将推理 token 折叠到输出 usage 中；当提供方上报该拆分时，`usage.reasoning` 会映射为 `reasoningTokens`——它是 `outputTokens` 的子拆分，绝不是新增的桶。
...
只有在 loop 记录生成内容后，它才会影响后续输入。推理 token 始终留在输出 usage 内；提供方上报的推理拆分会作为 `reasoningTokens` 一并记录，且不改变计费桶。
...
- **认证恢复只发生在内容之前，且属于适配器本地**：在任何内容之前到达的提供方凭据拒绝（HTTP 401/403）默认获得一次额外尝试：适配器会强制刷新该路由已存储的 OAuth 凭据——pi-ai 自己的刷新只在凭据过期时触发，因此被提供方提前拒绝的令牌永远不会获得刷新——等待 `authRecovery.delayMs`（默认 1000）后重试。刷新失败不会阻止重试；令牌端点故障并不能说明资源端点是否仍拒绝已存储的凭据。预算按路由配置（`authRecovery.retries`，默认为 1；`0` 表示禁用），重试会从凭据存储重新解析认证，被放弃尝试的末尾 usage 块也随之一并丢弃。任何已有流式内容之后的失败，或预算耗尽之后的失败，都会原样到达轮次层，在那里 `retryPolicy` 仍可为 `AUTH` 代码安排完整的轮次级重试。
