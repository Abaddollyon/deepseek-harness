...
思考级别如何在协议中传输——单独一个 `reasoning_effort`、DeepSeek 的 `thinking: {type}` 加上档位、z.ai 的 `thinking` 对象，诸如此类——就是 pi-ai 的 `compat.thinkingFormat`，pi-ai 会从端点 URL 猜测它；私有网关的 URL 什么也说明不了，于是说 DeepSeek 方言的网关只会收到 OpenAI 方言的请求，且无从更正。因此 `compat.thinkingFormat`、`compat.supportsReasoningEffort` 与 `compat.supportsDeveloperRole` 既可配置在路由上（作为其模型的默认值），也可按模型配置（逐字段胜出），解析顺序为模型 → 路由 → 已安装 catalog 条目 → pi-ai 按 URL 得出的猜测；`supportsDeveloperRole` 决定推理模型的 system prompt 使用 pi-ai 的 `developer` 还是 `system` 角色，因此拒绝 `developer` 的端点应设为 false。设置路由级开关会为路由上的每个模型遮蔽 catalog 条目的值，而且除了重述其值，没有任何写法能把某个字段交还给 catalog。`thinkingFormat` 接受 pi-ai 可分派的各种格式，但不含两个 `chat-template` 变体：它们需要的 `chatTemplateKwargs` 本配置并不暴露。三个开关都只存在于 `openai-completions` 上——其余协议的推理形状由协议本身承载——因此在其他协议的模型上设置模型级开关会使解析失败，路由级开关会跳过其他协议的模型，而完全没有 `openai-completions` 模型的路由则会被拒绝。pi-ai compat 面的其余部分（`supportsStore`、`maxTokensField`……）保持自动检测，特意不在此处开放配置。
...
该集合接收一个稳定的、由 DSH 拥有的 pi-ai 凭据存储。它在 `$DSH_HOME/credentials/pi-ai-v1` 下为每个提供方保存一份带版本、仅属主可读写的记录，通过提供方级进程内队列与文件锁串行化刷新，而 `list()` 只返回提供方与凭据类型元数据。因此，未设置 `apiKeyEnv` 的路由可以使用提供方原生 OAuth，并持久化轮换后的 token。设置了 `apiKeyEnv` 的路由继续使用 Harness 原有的明确失败凭据解析，并把结果作为 pi-ai 优先级最高的逐请求 `apiKey` 覆盖。
...
- **OAuth 登录仍由宿主负责**：适配器会存储并刷新提供方原生 OAuth 凭据，也会列出支持 OAuth 的 catalog 路由，包括 `openai-codex` 与 `anthropic`，但不会在 LLM 能力上暴露交互式登录流程。受信任的 Node 宿主必须驱动 pi-ai 的 `AuthInteraction`，并通过 `PiAiOAuthCredentialStore` 写入；凭据、刷新响应与原始提供方错误不得进入浏览器载荷或 session 回放。
