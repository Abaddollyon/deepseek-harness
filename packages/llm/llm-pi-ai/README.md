...
- **Auth recovery is pre-content and adapter-local** — a provider credential rejection (HTTP 401/403) that arrives before any content earns one extra attempt by default: the adapter forces a refresh of the route's stored OAuth credential — pi-ai's own refresh only fires on an expired credential, so a token a provider rejects early never earns one — waits `authRecovery.delayMs` (default 1000), and retries. A refresh failure does not stop the retry; a token-endpoint outage says nothing about whether the resource endpoint still rejects the stored credential. The budget is per route (`authRecovery.retries`, default 1; `0` disables), the attempt replays against the credential store afresh, and the abandoned attempt's terminal usage chunk is dropped with it. A failure after any streamed content, or after the budget is spent, reaches the turn unchanged, where `retryPolicy` can still schedule a full turn-level retry for the `AUTH` code.
