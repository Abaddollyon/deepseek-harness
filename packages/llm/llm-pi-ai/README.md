...
The collection receives one stable DSH-owned pi-ai credential store. It persists one versioned owner-only record per provider under `$DSH_HOME/credentials/pi-ai-v1`, serializes refresh through provider-scoped process and file locks, and returns only provider/type metadata from `list()`. A route that omits `apiKeyEnv` may therefore use provider-native OAuth and persist a rotated token. A route that names `apiKeyEnv` keeps the existing fail-loud Harness credential resolution and passes the result as pi-ai's highest-priority per-request `apiKey` override.
...
- **OAuth login remains host-owned** — the adapter stores and refreshes provider-native OAuth credentials and offers OAuth-capable catalog routes, including `openai-codex` and `anthropic`, but it does not put an interactive login flow on the LLM capability. A trusted Node host must drive pi-ai's `AuthInteraction` and write through `PiAiOAuthCredentialStore`; credentials, refresh responses, and raw provider errors must not enter browser payloads or session replay.
