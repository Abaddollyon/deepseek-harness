...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` 是唯一一份共享的环境清理定义：环境中形似凭据的名称与 `DSH_*` 名称都会被丢弃，显式 `env` 在清除之后合并。`sdkManagedSpawnSpec()`、`ManagedSdkProcess` 和 `disposeManagedSubprocess()` 将官方 SDK 的自定义 spawn hook 投射到同一套环境策略、受管进程树所有权、生命周期事件和完全停稳的清理流程上。
...
- **SDK 的默认 spawn 仍在服务之外**：没有自定义 spawn hook 的 SDK 传输无法把内部进程路由到本服务。提供该 hook 的 SDK 可以使用导出的受管投射 helper，而无需重复实现进程所有权。
