...
- `scrubbedParentEnv()` / `SENSITIVE_ENV_PATTERN` are the one shared scrub definition: ambient credential-shaped and `DSH_*` names are dropped, and explicit `env` merges after the scrub. `sdkManagedSpawnSpec()`, `ManagedSdkProcess`, and `disposeManagedSubprocess()` project official SDK custom-spawn hooks onto the same environment policy, managed process-tree ownership, lifecycle events, and quiescent teardown.
...
- **SDK default spawns remain outside** — an SDK transport without a custom-spawn hook cannot route its internal process through this service. SDKs with such a hook can use the exported managed projection helpers instead of duplicating process ownership.
