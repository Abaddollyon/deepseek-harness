...
每个生成的 workspace 都是封闭的：harness 将其锚定为自身的项目根，launcher 丢弃继承的 `DSH_*` 值（两者均由 [`dsh-loader-smoke`](../loader-smoke/README.md) 拥有），此外还有按运行隔离的 `DSH_HOME` 和 `DSH_AGENTS_HOME`。因此一次运行不会吸收宿主机上的任何 `AGENTS.md`、`CLAUDE.md`、已安装 skill 或导出的部署变量，放在 `os.homedir()` 下的场景与放在平台临时根目录下的场景记录相同的日志。需要工作区指令的场景以 `workspace/AGENTS.md` 形式签入；需要 skill 的场景签入 `workspace/.dsh/skills/<name>/SKILL.md`（`skill-load` 场景），或让组合指向仓库拥有的 skill 目录。
