...
Every generated workspace is hermetic: the harness anchors it as its own project root and the launcher drops inherited `DSH_*` values ([`dsh-loader-smoke`](../loader-smoke/README.md) owns both), on top of the per-run `DSH_HOME` and `DSH_AGENTS_HOME`. A run therefore absorbs no `AGENTS.md`, `CLAUDE.md`, installed skill, or exported deployment variable from the host, and a scenario placed under `os.homedir()` records the same log as one under the platform temp root. A scenario that wants workspace instructions ships them as `workspace/AGENTS.md`; one that wants skills ships `workspace/.dsh/skills/<name>/SKILL.md` (the `skill-load` scenario) or points the composition at a repository-owned skill directory.
