  ...
  try {
    await vi.waitFor(async () => {
      ...
      if (log === undefined || !latestTurnIsClosed(log.content)
        ...
        || !hasClosedTurn(log.content, minimumTurn)) throw new Error(diagnostic)
    }, { interval: WAIT_POLL_INTERVAL_MS, timeout: timeoutMs })
  } catch (error) {
    if (error instanceof Error && error.message === 'Timed out in waitFor!') throw new Error(diagnostic)
    throw error
  }
}
