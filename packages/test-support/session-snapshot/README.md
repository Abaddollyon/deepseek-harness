...
`dsh-session-snapshot` provides the shared support behind keyless recorded-session tests (`pnpm run test:snapshot`): closed manifests, typed identity redaction, normalization, workspace comparison, fixture guards, and protocol adapters for headless, SDK, ACP, and Web owners. The ACP adapter launches the tested profile as a real subprocess from an owned cwd marked with an empty `.git` directory, drives a deterministic input script, and registers the complete record, replay, and refresh suite. Every scenario owns enough committed evidence to prove model-visible output and filesystem effects without trusting the agent's report. The package entry imports vitest and is therefore available only inside a vitest run.
...
Retained historical headless and SDK scenarios keep their canonical Session files unchanged and selected for replay, with no canonical V3 sibling in the pinned directory. Their exact normalized native V3 output is recorded separately in `writer.expected.jsonl` for the parent and `writer.<ordinal>.expected.jsonl` for children; these are output oracles, not replay generations. Retained SDK scenarios use `notifications.current.expected.jsonl` for current protocol output. Comparisons neither project current events back into a historical format nor strip structural differences. Independent migration tests verify the official transform rather than treating native writer layout as its expected event sequence. The [Web preset-migration scenario](../../../apps/web/tests/preset-migration.snapshot.ts) directly asserts its published storage successor instead of maintaining an unused native-writer oracle.

Headless and ACP owners may declare `writerOracle: separate` when current-format replay input must remain immutable while native writer output changes. This declaration cannot coexist with `session` or historical `sessionFormat`. Every selected replay role must use the current format, and each must have exactly one current-format `writer[.<ordinal>].expected.jsonl` output oracle. Missing, extra, noncanonical, and undeclared writer files fail corpus validation. Refresh updates the separate oracles; replay compares the complete normalized current output without removing event or descriptor differences. Headless record mode skips these scenarios, and canonical generation publication refuses an existing destination. Historical coverage and retention limits remain independent.

ACP scenario tables mirror the manifest with `Scenario.writerOracle: 'separate'`. This setting requires full Session-log comparison even when `comparesLog` is false. Eligible live recording and keyless refresh write the separate writer files while preserving replay inputs; replay validates their exact inventory and current-format headers before launching the profile. Writer output also supplies request-header pins and must satisfy storage and normalization fixed-point checks.

### Record, replay, and refresh

`pnpm run test:snapshot:record` calls the live LLM and writes the harvested current generation under its canonical versioned filename. Committed Session generations remain immutable, including generations of a child role absent from a later run; a verified current replacement does not authorize moving, overwriting, or deleting its predecessor. Scenarios with an explicit `sessionFormat` remain read-only in record mode. `pnpm run test:snapshot:refresh` stays keyless, runs the selected highest replay input, and writes stdout, owned prompt and tool-schema sidecars, and a fresh current-generation comparable Session output; retained historical scenarios write the separate writer-output oracles instead of a canonical V3 replay generation. Each composition owner keeps its replay patch beside its live patch; top-level `snapshots/` owns Session-driven scenarios, while other expected outputs stay beside their owning package. [`dsh-llm-replay`](../llm-replay/README.md) serves the recorded streams selected through `DSH_SNAPSHOT_*` environment values.

Materialized profile patches preserve bare plugin names and link packages into the temporary profile fallback. Package resolution prefers the authored patch's installation, then only packages declared in the snapshot harness's dependencies, devDependencies, or peerDependencies, so replay-only packages remain available under plain Node without entering shipped profile dependencies.
