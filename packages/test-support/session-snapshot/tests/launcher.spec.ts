...
const manifestFixture = vi.hoisted(() => ({ omitDependencySections: false }))
vi.mock('node:fs', async (importOriginal) => {
  const actual = await importOriginal<typeof import('node:fs')>()
  return {
    ...actual,
    readFileSync: (...args: Parameters<typeof actual.readFileSync>) => {
      if (manifestFixture.omitDependencySections && String(args[0]).replaceAll('\\', '/').endsWith('/session-snapshot/package.json')) {
        return '{}'
      }
...
const roots: string[] = []
afterEach(async () => {
  manifestFixture.omitDependencySections = false
...
describe('snapshot profile package links', () => {
  ...
  it('resolves patch-local packages without optional harness dependency sections', async () => {
    ...
    manifestFixture.omitDependencySections = true
    const materialized = materializeProfilePatch(source, cwd, target, 0)
