...
describe('workspaces action face', () => {
  it('records every IWorkspaces verb with inert defaults and honors stubs', async () => {
    ...
    ws.retryFeed()
    expect(ws.calls.at(-1)?.method).toBe('retryFeed')
    ...
    expect(ws.calls.map(c => c.method)).toEqual(
      ['retryFeed', 'create', 'create', 'rename', 'updatePaths', 'updatePaths', 'delete', 'insertBefore', 'insertSessionBefore', 'archiveSession'])
    ...
    ws.stub('updatePaths', () => Promise.resolve({ workspaceId: 'w1', title: 'P', path: '/p', additionalPaths: ['/stub'], sessionIds: [] } as never))
    ...
    expect((await ws.updatePaths('w1' as WorkspaceId, ['/stub'])).additionalPaths).toEqual(['/stub'])
    await ws.delete('w1' as WorkspaceId)
