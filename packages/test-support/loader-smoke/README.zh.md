...
`runLoaderSmoke` 接受可执行文件路径和配置路径、可选的完整可执行文件参数、环境变量覆盖、标准输入、运行前准备和清理前检查。它负责隔离工作目录及其播种的项目根标记（`isolateWorkspaceProjectRoot`，使工作区指令与项目技能发现不会向上走入临时 cwd 之外的宿主根）、DSH 主目录、诊断、截止时间、终止、EOF 和清理；进程以零状态退出后返回两个流，失败时则返回拒绝并附带两个流。
