...
[已提交语料测试](tests/session-format-corpus.spec.ts) 通过真实 catalog 还原 `snapshots/`、`packages/` 与 `scripts/snapshots/python-sdk-single-exe/` 下每个带版本的 `session*.jsonl`，且不改变源字节。其[清单](tests/session-format-corpus-inventory.ts) 按路径、源代际、错误类型与精确原因固定有意拒绝的历史转换；拒绝消失或变化都会使测试失败。当前代际产物不能获得例外。独立的无效前代清单以 SHA-256、源代际、精确的 `Error` 类和消息固定不可变的畸形 fixture；其版本最高的同组代际必须是当前版本，且能够成功还原。agent-instructions 的 v0 前代包含手工编写但没有匹配开始事件的压缩检查点，因此仍被拒绝，不会补入虚构的生命周期事件。没有版本 header 的快照框架协议示例具有独立的显式豁免。其他所有还原错误都携带产物路径并使测试失败；历史文件保持不变，原生当前 fixture 则由 owner 修正。
