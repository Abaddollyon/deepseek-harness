...
`turn/*`, `step/*`, `system/message`, `user/message`, `assistant/message`, `assistant/attempt`, and `tool/*` are durable session events; the rest are live extension points across three domains. `agent/assistant-stream` publishes process-local start, transient chunk, and end frames. The loop commits the complete compact stream as one message or log-only attempt before a committed end frame, and the Web Session-follow adapter is the live event's only remote consumer. `agent/pre-step`, `agent/request`, `agent/request-preflight`, `llm/stream`, and the three `tools/*` events are waterfalls, whose listeners must call `next()` to delegate; `agent/turn-stopping` is serial and has no `next()`.

Input reaches the driver through one inbox. Some messages wake it immediately; injected context waits in the inbox until another message does.

`agent/pre-step` accepts, rewrites, or rejects claimed input; a rejected or empty first claim closes a durable turn without a step. Enter decisions may declare `startsRequestSeries`; wrapping listeners preserve it. After assembly and `step/start`, `agent/request` and `prepareCall()` resolve the route; cancellation during either phase commits neither system nor user messages. Each attempt reconciles the retained prompt using the prepared capability, admits users once, logs header/context, and runs `agent/request-preflight` before deriving and freezing the request. Retries repeat neither assembly nor input admission. System prompts exist only as `system/message` history; clearing, cached-prefix updates, and series consolidation follow the [system-node decision](../.agents/notes/implemented/architecture/2026-09-02-system-prompt-as-surface-node.md).

The loop sends immutable requests with live cancellation; [agent-loop](../packages/core/agent-loop/README.md#understand-the-implementation) owns request construction, request-series header representation, prompt reconciliation, and resume semantics.

Optional `AgentOptions.budget` limits steps, input, output, and retries for one live loop ([semantics](subsystems/core.md)). Preflight requires a newer declared `surface.replaceGeneration` to retry and admits after eight productive retries so provider-error recovery remains reachable. Productive replacements consolidate the system prompt and begin a new request series.
