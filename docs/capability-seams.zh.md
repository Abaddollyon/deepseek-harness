...
| `ctx.jobs` | `seam` | [`jobs`](../packages/jobs/jobs) | [`jobs-local`](../packages/jobs/jobs-local) | [`tool-bash`](../packages/shell/tool-bash), [`tool-terminal`](../packages/terminal/tool-terminal), [`tool-subagent`](../packages/subagent/tool-subagent), [`tool-jobs`](../packages/jobs/tool-jobs), [`run-supervisor`](../packages/jobs/run-supervisor) | - | 生产方（后台 bash、PTY 发送和 subagent 委派）登记正在运行的工作；tool-jobs 是面向模型的控制器，用于读取、列出和终止这些工作；jobs-local 是进程本地注册表；run-supervisor 在启动时对持久记录做 reconcile。 |
| `ctx.jobStore` | `seam` | [`jobs-store-domain`](../packages/jobs/jobs-store-domain) | [`jobs-store-domain`](../packages/jobs/jobs-store-domain) | [`jobs-local`](../packages/jobs/jobs-local), [`run-supervisor`](../packages/jobs/run-supervisor) | - | jobs-store-domain 声明 JobStore seam 并在 storage domain 形态上提供它；jobs-local 在启用 persist 时把记录镜像到这里，并在启动时恢复它们；run-supervisor 读取恢复出的记录以完成 reconcile。 |
