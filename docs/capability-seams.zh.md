...
| `ctx.jobStore` | `seam` | [`jobs-store-domain`](../packages/jobs/jobs-store-domain) | [`jobs-store-domain`](../packages/jobs/jobs-store-domain) | [`jobs-local`](../packages/jobs/jobs-local) | - | jobs-store-domain 声明 JobStore seam 并在 storage domain 形态上提供它；jobs-local 在启用 persist 时把记录镜像到这里，并在启动时恢复它们。 |
