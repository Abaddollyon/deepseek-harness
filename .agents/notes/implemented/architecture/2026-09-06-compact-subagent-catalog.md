...
**Show every inactive sibling as a compact row.** Even one-line rows overwhelm a catalog with hundreds of agents. Folding history keeps active work visible while preserving access to every record.
...
More historical agents fit in the scrollable menu and active branches remain easy to reach. Opening a branch conversation uses the explicit Open action. Component regressions cover a 114-agent catalog, resumed activity, ordering, compaction, and keyboard navigation; the persisted-subagent browser scenario checks branch expansion, one-line geometry, and the assembled accessible tree.
