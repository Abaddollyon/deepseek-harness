...
A per-agent effort binds only for in-process children. The shipped `spawn` backend applies it; a remote backend that ignores `agentOptions` ignores the effort exactly as it already ignores `provider` and `model`, and host-side validation still runs, so the request is checked against a route the child may not use. Closing that gap is the capability-flag alternative above.
