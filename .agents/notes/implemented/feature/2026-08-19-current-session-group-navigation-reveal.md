...
Workspace groups in the sidebar start closed. The only automatic expansion was a standing `Object.hasOwn(groupExpansion, currentGroup)` effect in `SessionTree`: it opened the current Session's group the first time that group appeared in the persisted record and could never tell a user's deliberate fold from a group it had simply not seen yet, which is why it was removed. That left the group header toggle and the group's ＋ button as the only gestures that open a group, and nothing at all opened the group holding the Session the user is actually in. Selecting a Session from anywhere but that header — startup selection, a fresh Workspace connect, a fork, a Host-side selection change — left its row inside a folded group, so the sidebar showed the user a closed folder and no route back to the conversation on screen. Collapsed groups pin running rows, but pinning covers live work only: an idle current Session in a folded group had no representation at all.
