...
Normal completion checks whether focus is inside the content before closing. Focused content remains mounted with current completed status and closes after focus leaves. When a navigable member's child row leaves the ordinary Session list while its button holds focus, `MemberRow` keeps the same button mounted as `aria-disabled` until blur; the row then renders as ordinary non-interactive text. This preserves the active DOM target without a focus manager. Settlement alone no longer removes navigability — a settled member stays openable while its child row is listed ([Workflow terminal members stay navigable](../bug-fix/2026-08-25-workflow-terminal-member-navigation.md)) — so the retention trigger is the row's disappearance, not the member's terminal transition.
