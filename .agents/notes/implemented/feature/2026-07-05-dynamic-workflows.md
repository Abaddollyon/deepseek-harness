...
- **Nested `workflow()`**, **token `budget`**, and the `isolation`/`agentType` agent options (each rejects loud with a message naming it deferred). The per-call reasoning effort shipped later as `reasoningEffort`, deliberately not under Claude Code's bare `effort` spelling: [per-agent reasoning effort](2026-08-19-workflow-per-agent-reasoning-effort.md).
