...
`dsh-tool-workflow` adds `ownership: caller | supervisor`, defaulting to `caller`. Caller ownership preserves the existing request signal, abort bridge, return shape, errors, disposal order, and model-facing description. Supervisor ownership omits the caller signal, registers a parent-owned `workflow` job with durable owner Session identity and no resume specification, appends `run/detached`, and immediately returns `{ runId, jobId, status: "running" }`. The job cancel hook calls `WorkflowRun.cancel`; its final-output promise awaits the result and disposal before closing the recorder and exposes the existing bounded completion rendering through `job_output`.
