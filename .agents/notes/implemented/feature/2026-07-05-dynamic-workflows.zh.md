...
- **嵌套 `workflow()`**、**token `budget`**，以及 `isolation`/`agentType` agent 选项（每个都会明确拒绝，并在消息中注明其已延迟实现）。按调用的推理等级已在后续以 `reasoningEffort` 落地，并刻意不沿用 Claude Code 的裸名 `effort`：见[按 agent 的推理等级](2026-08-19-workflow-per-agent-reasoning-effort.md)。
