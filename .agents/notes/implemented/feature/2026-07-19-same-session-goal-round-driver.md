...
The `agent/pre-step` waterfall is the entry fence. A positive goal source enters only when its exact `MessageId`, identity, and content match the driver's pending reservation, the live goal still has that id and revision, activation remains armed, and the round is still the next number. The plugin checks once before delegating and again after downstream listeners return. This second check prevents an async listener from editing or pausing the goal while still entering the old prompt.
