...
工作流进度与父对话保存在同一日志中，能跨刷新与进程恢复，原工具卡保持不变。Supervisor 收养保持非对称的账目状态机：已有的 `run/abandoned` 可以满足后续 `run/resumed` 重试，但已有的 `run/resumed` 永远不能满足已收养工作流被取消时所需的 `run/abandoned` 账目；被取消的工作流还需要持久成员／运行 closer，只有 account 与 closure append 确认写入或已存在后才清除 adoption marker。恢复的 stopping 工作流会终态化为 killed，以 abandoned 记账，不重启且不发送通知，并继续使用同一串行离线 append 通道。[受监督的工作流所有权](2026-08-29-supervised-workflow-ownership.zh.md)通过增加到 Jobs 的显式可选移交，部分取代了本说明原有的执行所有权结论。持久协议增加六类小事件和一个包所有的 invariant；首次写入失败会刻意牺牲后续观察，而不是牺牲工作流正确性。浏览器 State 按已加载窗口派生，状态驱动的 disclosure 生命周期把复盘选择留在本地，导航会随列表事实消失。设计保留运行时阶段与叙述观察以及真实成员状态，同时放弃静态图推断、输出和控制操作。
