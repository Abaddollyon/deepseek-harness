...
**Capture the `cause` via a pi-ai transport hook.** Rejected. Current pi-ai's `StreamOptions` (through `ProviderRequestOptions`) exposes three hooks: `onPayload` only inspects or rewrites the outbound payload, and `onResponse` fires before the body stream is consumed, so neither can observe a mid-stream drop. A custom `fetch` implementation could capture undici's `cause` at the transport layer, but the captured value would then have to travel from the wrapper to the terminal-event mapping through adapter-owned per-request side state, keyed across concurrent streams and pi-ai's own client retries (`maxRetries`) — a side channel that can attribute a failure to the wrong request — while pi-ai still flattens the terminal event to `error.message`, splitting classification authority across two places. The hook also does not cover WebSocket transports, and pi-ai warns that provider adapters may reject a custom `fetch`. Text classification stays the single best-effort authority until pi-ai forwards the original `Error`.
